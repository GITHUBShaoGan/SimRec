...
public class PswdModelImpl implements PswdModel {
    @Override
    public void loadPass(int sortType, long pageNo, long pageSize, OnPassLoadListener onPassLoadListener) {
        if (pageNo < 1 || pageSize < 1) {
            onPassLoadListener.onLoadError(ResUtils.getString(R.string.error_size));
            return;
        }
        List<Password> passwordList = null;
        try {
            passwordList = PassDao.getInstances().queryByPage(sortType, pageNo, pageSize);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (passwordList == null) {
            onPassLoadListener.onLoadError(ResUtils.getString(R.string.error_query_failed));
        } else {
            if (passwordList.size() < pageSize) {
                onPassLoadListener.onLoadSuccess(DataLoadType.END, passwordList);
            } else {
                onPassLoadListener.onLoadSuccess(DataLoadType.FINISH, passwordList);
            }
        }
    }
